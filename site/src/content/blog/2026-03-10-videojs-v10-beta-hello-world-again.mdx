...
When you're ready to go deeper, add the files for any skin to your project and get the full code in your framework's language — real components you can modify, inspired by [shadcn/ui](https://ui.shadcn.com/). At Beta launch, that meant copying the code from the docs; today, the `shadcn` CLI handles it.
...
- [llms.txt](https://videojs.org/llms.txt) to make navigating Video.js docs more efficient, including [framework-specific llms.txt](/docs/framework/react/llms.txt)
- Markdown versions of every [individual doc](/docs/framework/react/reference/components/play-button.md). If your agent hits our site with the accept: text/markdown header — as many like Claude Code do — we'll send the markdown version of the page, saving your agent loads of unnecessary context bloat.
...
The migration guides mentioned here (and under "What's coming" below) are now available: [Video.js 8](/docs/framework/html/guides/migrate-from-video-js-8), [Mux Player](/docs/framework/html/guides/migrate-from-mux-player), [Plyr](/docs/framework/html/guides/migrate-from-plyr), and [Media Chrome](/docs/framework/html/guides/migrate-from-media-chrome).
</Aside>
